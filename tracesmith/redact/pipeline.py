...
from tracesmith.redact.discovery import discover_private_terms
...
def run_redact(in_dir: Path, out_dir: Path, config: RedactorConfig) -> dict[str, Any]:
    ...
    if config.privacy_filter:
        ...
        from tracesmith.redact.privacy_filter import (
            PrivacyFilter,
            privacy_filter_pass,
        )
    ...
    if config.gitleaks_fix:
        from tracesmith.redact.gitleaks import scrub_findings
        layers["gitleaks_fix"] = scrub_findings(out_dir)

    if config.llm_residue_url:
        from tracesmith.redact.llm_residue import llm_residue_pass
        layers["llm_residue"] = llm_residue_pass(
            out_dir, base_url=config.llm_residue_url
        )

    if config.gitleaks:
        from tracesmith.redact.gitleaks import run_scan
